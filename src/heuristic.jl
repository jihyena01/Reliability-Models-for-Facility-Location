...
function heuristic_RPMP(LR_X, LR_Y, I, J, P, d, q, NF, alpha)
    ...
    w2 = sum(h[i] * (sum(d[i,j]* q^r * heuristic_Y[i,j,(r)] for j in NF, r in 1:(P)) + sum(d[i,j] * q^r * (1-q) * heuristic_Y[i,j,(r)] for j in F, r in 1:(P)) ) for i in I)
    heuristic_val = alpha * w1 + (1-alpha) * w2
    return LR_X, heuristic_Y, heuristic_val
end


function heuristic_RFLP(LR_X, LR_Y, I, J, P, f, d, q, NF, alpha)
    heuristic_X = get_used_facilities(LR_Y)
    heuristic_Y = zeros(Int, length(I), length(J), P) # r index 1부터 시작!
    ...
    w1 = sum(f[j] * X[j] for j in J) + sum(h[i] * d[i,j] * heuristic_Y[i,j,1] for i in I, j in J)
    w2 = sum(h[i] * (sum(d[i,j]* q^r * heuristic_Y[i,j,(r)] for j in NF, r in 1:(P)) + sum(d[i,j] * q^r * (1-q) * heuristic_Y[i,j,(r)] for j in F, r in 1:(P)) ) for i in I)
    heuristic_val = alpha * w1 + (1-alpha) * w2
    return LR_X, heuristic_Y, heuristic_val
end
